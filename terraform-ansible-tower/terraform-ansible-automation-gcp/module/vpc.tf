...
######################################################## Firewall Rule to allow Receptor Service ######################################################

resource "google_compute_firewall" "receptor_service" {
  name          = "allow-receptor-service"
  direction     = "INGRESS"
  network       = google_compute_network.gcp_vpc.id
  source_ranges = ["${var.ip_public_range_subnet}", "${var.ip_range_subnet}"]  ### Google uses specific IP ranges for its health check probes.
  allow {
    protocol = "tcp"
  }
  target_tags = ["allow-receptor-service"]
}
